      ...
      <main className="flex px-20 py-7 bg-[#f5f5f5] h-[580px] gap-5">
        ...
        <div className="flex-[80%] flex flex-col items-center gap-3 pt-5 p-8 bg-[#dac387] rounded-xl shadow overflow-y-auto">
          <div className="w-full mb-14">
            ...
            <div className="w-[340px] text-stone-900">Add at least one phone number for the transfer ID so you can start transfering your money to another user.</div>
          </div>
          {successMessage ? (
          ...
          <form onSubmit={upPhoneNum} className="flex flex-col gap-10 w-6/12">
            <div className={`flex gap-3 items-center border-b-2 ${contentPhoneNumber ? "border-[#e3d0a1]" : "border-[#e3d0a1]"}`}>
              <BsTelephone className={`text-xl ${contentPhoneNumber ? "text-[#fdfdfd]" : null}`} />
              <div className="font-bold">+62</div>
              <input
                onChange={(e) => setContentPhoneNumber(e.target.value.length)}
                className="bg-[#dac387] w-full py-2 focus:outline-none no_arrows"
                type="number"
                name="phoneNumber"
                id="phoneNumber"
            ...
            <div>
              <button type="submit" className={`w-full rounded-xl font-bold py-3 ${contentPhoneNumber ? 'bg-[#857752] text-white': 'bg-[#E5E5E5]'}`}>Continue</button>
            </div>
          </form>
        </div>
      </main>
